...
export function useAttendanceHistory(categoryId: string | null) {
  const { organization } = useAuth();

  const { data: history = [], isLoading } = useQuery({
    queryKey: ['attendance-history', categoryId, organization?.id],
    queryFn: async () => {
      ...
      const grouped = (data || []).reduce((acc, record) => {
        if (!acc[record.date]) {
          acc[record.date] = { total: 0, present: 0, absent: 0, justified: 0 };
        }
        acc[record.date].total++;
        if (record.status === 'presente') acc[record.date].present++;
        else if (record.status === 'ausente') acc[record.date].absent++;
        else if (record.status === 'justificado') acc[record.date].justified++;
        return acc;
      }, {} as Record<string, { total: number; present: number; absent: number; justified: number }>);
